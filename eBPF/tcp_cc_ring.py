...
# process event
def evt_poll_ring_data(ctx, data, size):
    global ebpf_info
    ...
    stats_content = "{:<16} {:<16} {:<6} f{:<2} - {:<16}:{:<6} -> {:<16}:{:<6} - {:<8} {:<12} {:<8} {:<12} {:<8} {:<8} {:<8}".format(
        time_s, event.comm.decode('utf-8'), event.pid,
        event.func_id,
...
def ebpf_ring_buffer_show():
    ...
    try:
        ebpf_info.stats_file_handle = open(ebpf_info.stats_file, "w", encoding='UTF-8')
        ebpf_info.b["events"].open_ring_buffer(evt_poll_ring_data)
        while 1:
            ebpf_info.b.ring_buffer_poll()
